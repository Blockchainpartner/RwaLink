...
task('lz:oapp:send', 'Send RWA cross‐chain using MyOApp contract')
    ...
    .setAction(async (args: { dstEid: number; amount: number; options?: string }, hre: HardhatRuntimeEnvironment) => {
        ...
        let messagingFee
        try {
            messagingFee = await myOAppContract.quoteSendRWA(
                args.dstEid,
                args.amount,
                options,
        ...
        try {
            tx = await myOAppContract.sendRWA(args.dstEid, args.amount, options, {
        ...
        DebugLogger.printLayerZeroOutput(
            ...
            `Successfully sent "${args.amount}" from ${hre.network.name} to ${endpointIdToNetwork(args.dstEid)}`
        )
