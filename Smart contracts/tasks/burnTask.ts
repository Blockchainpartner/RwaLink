...
task('lz:oapp:burn', 'Burn cross‐chain using MyOApp contract')
    ...
    .setAction(async (args: { dstEid: number; dstEid2: number; address: string; amount: number; options?: string }, hre: HardhatRuntimeEnvironment) => {
        ...
        logger.info(`Address to burn: "${args.address}"`)
        logger.info(`Amount to burn: "${args.amount}"`)
        ...
        let messagingFee
        try {
            messagingFee = await myOAppContract.quoteBatchBurn(
        ...
        try {
            tx = await myOAppContract.batchBurn([args.dstEid, args.dstEid2], args.address, args.amount, options, {
