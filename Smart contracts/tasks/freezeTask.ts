...
task('lz:oapp:freeze', 'Freeze tokens cross‐chain using MyOApp contract')
    ...
    .setAction(async (args: { dstEid: number; dstEid2: number; address: string; amount: number; options?: string }, hre: HardhatRuntimeEnvironment) => {
        ...
        let messagingFee
        try {
            messagingFee = await myOAppContract.quoteBatchFreeze(
        ...
        try {
            tx = await myOAppContract.batchFreeze([args.dstEid, args.dstEid2], args.address, args.amount, options, {
